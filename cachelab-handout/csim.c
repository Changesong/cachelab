...
typedef struct _cacheset {  //lines -> cache
    cacheline_t* lines;
} cacheset_t;
...
{
    int lru = set.lines[0].recently_used;
    int lru_index = 0;
    int i;
...
int accessCache(cache_t cache, uint64_t address)
{
    ...
    if (empty_index != -1) {
        ...
        cur_set.lines[empty_index].is_valid = true;
...
int main(int argc, char** argv)
{
    ...
    printSummary(cInfo.hits, cInfo.misses, cInfo.evicts);
    fclose(tracefile);
    freeCache(cache);
